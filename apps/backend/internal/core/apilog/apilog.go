...
// New logs every request that passes through it to apiLogRepo, capturing
// method/path/status/duration/client info and the {success,error} envelope
// every handler in this app already returns — so new routes get logged for
// free, with no per-handler instrumentation needed.
