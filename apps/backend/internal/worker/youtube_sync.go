...
func StartYouTubeSync(uc domain.SongUsecase, logRepo domain.WorkerLogRepository, intervalMinutes int, countries []string) {
	log.Println("🚀 [Worker] เริ่มต้นระบบ Background Sync แล้ว...")

	interval := time.Duration(intervalMinutes) * time.Minute

	syncData(uc, logRepo, interval, countries)
...
func syncData(uc domain.SongUsecase, logRepo domain.WorkerLogRepository, interval time.Duration, countries []string) {
	acquired, err := logRepo.WithLock(syncLockKey, func() error {
		runSync(uc, logRepo, interval, countries)
		return nil
	})
...
func runSync(uc domain.SongUsecase, logRepo domain.WorkerLogRepository, interval time.Duration, countries []string) {
	for _, country := range countries {
		...
		logEntry := domain.WorkerLog{
			JobName:         domain.MusicSyncJobName,
			CountryCode:     country,
			StartedAt:       startedAt,
